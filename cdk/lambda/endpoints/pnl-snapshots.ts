  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      ...
      const values = snapshots.map(s =>
        `(${s.strategyId}, ${s.listingId}, ${s.netQuantity}, ${s.avgEntryPrice}, ${s.realizedPnl}, ${s.totalFees}, ${s.leavesBuyQty ?? 0}, ${s.leavesSellQty ?? 0}, ${s.sessionId ? `'${s.sessionId}'` : 'NULL'}, ${s.markPrice}, ${s.unrealizedPnl}, ${s.totalPnl})`
      ).join(', ');

      const query = `
        INSERT INTO pnl.snapshot
          (strategy_id, listing_id, net_quantity, avg_entry_price, realized_pnl, total_fees, leaves_buy_qty, leaves_sell_qty, session_id, mark_price, unrealized_pnl, total_pnl)
        VALUES ${values}
        RETURNING *;
      `;
      const result = await client.query(query);
